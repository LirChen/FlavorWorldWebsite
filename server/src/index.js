...
app.post('/api/recipes', upload.any(), async (req, res) => {
  try {
    ...
    const validMeatTypes = [
      'Vegetarian', 'Vegan', 'Chicken', 'Beef', 'Pork', 
      'Fish', 'Seafood', 'Lamb', 'Turkey', 'Mixed'
    ];
    
    const category = formData.category || 'Asian'; 
    ...
    const meatType = formData.meatType || 'Mixed'; 
    if (!validMeatTypes.includes(meatType)) {
      console.log('ERROR: Invalid meatType:', meatType);
      return res.status(400).json({ 
        message: `Invalid meat type: ${meatType}. Must be one of: ${validMeatTypes.join(', ')}` 
      });
    }
    
    const prepTime = Math.max(0, parseInt(formData.prepTime) || 0);
    ...
    console.log(' Creating recipe object with data:', {
    ...
    let enrichedRecipe = savedRecipe.toObject();
    try {
      if (mongoose.Types.ObjectId.isValid(savedRecipe.userId)) {
        const user = await User.findById(savedRecipe.userId);
        if (user) {
          enrichedRecipe.userName = user.fullName;
          enrichedRecipe.userAvatar = user.avatar;
          enrichedRecipe.userBio = user.bio;
        }
      }
    } catch (userError) {
...
app.get('/api/recipes/:id', async (req, res) => {
  try {
    console.log(' Fetching single recipe:', req.params.id);
    ...
    if (!mongoose.Types.ObjectId.isValid(req.params.id)) {
      console.log(' Invalid recipe ID format:', req.params.id);
      return res.status(400).json({ message: 'Invalid recipe ID' });
    }
...
app.delete('/api/recipes/:id', async (req, res) => {
  try {
    ...
    if (!deletedRecipe) {
      console.log(' Failed to delete recipe from database');
      return res.status(500).json({ message: 'Failed to delete recipe' });
    }

    console.log(' Recipe deleted successfully:', deletedRecipe.title);
    console.log(' Cleanup complete for recipe:', req.params.id);
